...
public:
	...
	glm::vec2 TransformScreenToWorld(glm::vec2 positionScreen)
	{
		...
		float x_transform = (positionScreen.x - (width / 2)) / (width / 2);
		float y_transform = ((height / 2) - positionScreen.y) / (height / 2);

		return glm::vec2(x_transform * aspect, y_transform);

		/* Answer Code
		
		const float aspect = float(width) / height;

		const float xScale = aspect *  2.0f / (this->width - 1);
